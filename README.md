...
* 얼마나 사용했는지를 다음 노드에 전파합니다. 다음 노드에서는 이전 노드, 전전 노드, 전전전 노드... 의 사용량을 고려해 사용할 수 있는 양을 산출하고, 그 안에서 원하는 만큼 사용합니다.
* 각 노드는 X = (`A_t1`, `B_t2`, `C_t3`, `D_t4`... `자기이전노드_바로이전시간`) 의 정보를 전달 받습니다.  
  모든 노드에서 X + `자기자신_현재시간` < L이 되도록 보장하면, 전체 사용량이 항상 `노드 개수` x L 이하임을 보장할 수 있습니다.
...
* **바로 이전 노드가 사용량을 독점하면, 다음 노드는 최대 사용량이 매우 낮아진다** :  
  우선 각 노드가 이전 노드로부터 남은 용량을 최대 `QUOTE%` 만큼만 사용할 수 있게 하여 다음 노드가 사용할 수 있는 최소 수치를 보장합니다.
...
* **리소스 사용을 위해 에포크를 기다리는 프로세스가 너무 느리다** :
  에포크가 끝나면 할당량을 만료시키지 않고, 에포크의 할당량별로 범위가 어느정도 겹쳐지게 합니다. 그러면 현재 사용량에 따라서 할당량을 미리 받을 수 있습니다.
